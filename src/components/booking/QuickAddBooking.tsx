...
export function QuickAddBooking() {
  ...
  const handleSubmit = async () => {
    ...
    if (isNewCustomer) {
      ...
      if (!validateIndianPhone(newCustomerPhone)) { toast.error('Enter a valid 10-digit phone number'); return; }
      try {
    ...
    if (!hallId) { toast.error('Please select a hall'); return; }
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            {selectedCustomer && !isNewCustomer ? (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-brand-50 border border-brand-100">
                ...
                <button onClick={() => { setSelectedCustomerId(null); setCustomerSearch(''); }} className="text-xs text-brand-600 font-medium">Change</button>
              </div>
            ) : isNewCustomer ? (
              <div className="space-y-3 p-4 rounded-xl bg-green-50 border border-green-100">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setIsNewCustomer(false)} className="text-xs text-green-600 font-medium">Cancel</button>
                </div>
                ...
                <input type="tel" placeholder="Phone (10 digits)" value={newCustomerPhone} onChange={(e) => setNewCustomerPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                  className="w-full px-3.5 py-2.5 rounded-lg border border-green-200 text-sm focus:ring-2 focus:ring-green-300 outline-none" />
              </div>
            ) : (
              <div className="relative">
                ...
                <button onClick={() => { setIsNewCustomer(true); setShowCustomerDropdown(false); }}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" placeholder="300" value={guestCount} onChange={(e) => setGuestCount(e.target.value)}
                className="w-full px-3.5 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-brand-200 outline-none" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" placeholder="3,50,000" value={totalAmount}
            ...
            <div>
              ...
              <input type="number" placeholder="87,500" value={advanceAmount} onChange={(e) => setAdvanceAmount(e.target.value)}
                className="w-full px-3.5 py-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-brand-200 outline-none" />
            </div>
          </div>
